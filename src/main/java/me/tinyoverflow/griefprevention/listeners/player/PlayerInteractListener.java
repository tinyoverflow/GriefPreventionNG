...
public class PlayerInteractListener implements Listener {
    private final List<Material> leftClickWatchedMaterialList = List.of(Material.OAK_BUTTON,
    ...
    void onPlayerInteract(PlayerInteractEvent event) {
        ...
        if (clickedBlock != null && plugin.getPluginConfig().getClaimConfiguration().getProtectionConfiguration().isLockContainersEnabled() && (event.getAction() == Action.RIGHT_CLICK_BLOCK && ((clickedBlock.getState() instanceof InventoryHolder && clickedBlock.getType() != Material.LECTERN) || clickedBlockType == Material.ANVIL || clickedBlockType == Material.BEACON || clickedBlockType == Material.BEE_NEST || clickedBlockType == Material.BEEHIVE || clickedBlockType == Material.BELL || clickedBlockType == Material.CAKE || clickedBlockType == Material.CARTOGRAPHY_TABLE || clickedBlockType == Material.CAULDRON || clickedBlockType == Material.WATER_CAULDRON || clickedBlockType == Material.LAVA_CAULDRON || clickedBlockType == Material.CAVE_VINES || clickedBlockType == Material.CAVE_VINES_PLANT || clickedBlockType == Material.CHIPPED_ANVIL || clickedBlockType == Material.DAMAGED_ANVIL || clickedBlockType == Material.GRINDSTONE || clickedBlockType == Material.JUKEBOX || clickedBlockType == Material.LOOM || clickedBlockType == Material.PUMPKIN || clickedBlockType == Material.RESPAWN_ANCHOR || clickedBlockType == Material.ROOTED_DIRT || clickedBlockType == Material.STONECUTTER || clickedBlockType == Material.SWEET_BERRY_BUSH))) {
            if (playerData == null) playerData = dataStore.getPlayerData(player.getUniqueId());
        ...
        //otherwise apply rules for doors and beds, if configured that way
                ...
                (plugin.getPluginConfig().getClaimConfiguration().getProtectionConfiguration().isLockWoodenDoorsEnabled() && Tag.DOORS.isTagged(
                        clickedBlockType) ||

                        plugin.getPluginConfig().getClaimConfiguration().getProtectionConfiguration().isLockSwitchesEnabled() && Tag.BEDS.isTagged(
                                clickedBlockType) ||

                        plugin.getPluginConfig().getClaimConfiguration().getProtectionConfiguration().isLockTrapDoorsEnabled() && Tag.TRAPDOORS.isTagged(
                                clickedBlockType) ||

                        plugin.getPluginConfig().getClaimConfiguration().getProtectionConfiguration().isLockLecternsEnabled() && clickedBlockType == Material.LECTERN ||

                        plugin.getPluginConfig().getClaimConfiguration().getProtectionConfiguration().isLockFenceGatesEnabled() && Tag.FENCE_GATES.isTagged(
                                clickedBlockType))) {
            if (playerData == null) playerData = dataStore.getPlayerData(player.getUniqueId());
            Claim claim = dataStore.getClaimAt(clickedBlock.getLocation(), false, playerData.lastClaim);
            if (claim != null) {
        ...
        else if (clickedBlock != null && plugin.getPluginConfig().getClaimConfiguration().getProtectionConfiguration().isLockSwitchesEnabled() && (Tag.BUTTONS.isTagged(
        ...
        else if (clickedBlock != null && plugin.getPluginConfig().getClaimConfiguration().getProtectionConfiguration().isLockContainersEnabled() && (clickedBlockType == Material.CAKE || Tag.CANDLE_CAKES.isTagged(
                clickedBlockType))) {
            if (playerData == null) playerData = dataStore.getPlayerData(player.getUniqueId());
            Claim claim = dataStore.getClaimAt(clickedBlock.getLocation(), false, playerData.lastClaim);
            if (claim != null) {
        ...
        else if (clickedBlock != null && (clickedBlockType == Material.NOTE_BLOCK || clickedBlockType == Material.REPEATER || clickedBlockType == Material.DRAGON_EGG || clickedBlockType == Material.DAYLIGHT_DETECTOR || clickedBlockType == Material.COMPARATOR || clickedBlockType == Material.REDSTONE_WIRE || Tag.FLOWER_POTS.isTagged(
        ...
        //otherwise handle right click (shovel, string, bonemeal) //RoboMWM: flint and steel
            ...
            for (Material material : Material.values()) {
                ...
                if (material.name().endsWith("_SPAWN_EGG")) spawn_eggs.add(material);
            ...
            if (clickedBlock != null && (materialInHand == Material.BONE_MEAL || materialInHand == Material.ARMOR_STAND || (spawn_eggs.contains(
                    materialInHand) && GriefPrevention.instance.getPluginConfig().getClaimConfiguration().getProtectionConfiguration().isPreventMonsterEggsEnabled()) || materialInHand == Material.END_CRYSTAL || materialInHand == Material.FLINT_AND_STEEL || materialInHand == Material.INK_SAC || materialInHand == Material.GLOW_INK_SAC || materialInHand == Material.HONEYCOMB || dyes.contains(
                    materialInHand))) {
            ...
            else if (clickedBlock != null && (materialInHand == Material.MINECART || materialInHand == Material.FURNACE_MINECART || materialInHand == Material.CHEST_MINECART || materialInHand == Material.TNT_MINECART || materialInHand == Material.HOPPER_MINECART) && !plugin.creativeRulesApply(
            ...
            //if he's investigating a claim
            else if (materialInHand == plugin.getPluginConfig().getClaimConfiguration().getToolsConfiguration().getInvestigationTool() && hand == EquipmentSlot.HAND) {
                ...
                if (player.isSneaking() && player.hasPermission("griefprevention.visualizenearbyclaims")) {
                    ...
                    BoundaryVisualization.visualizeNearbyClaims(player,
                ...
                //claim case
                    ...
                    //visualize boundary
                    BoundaryVisualization.visualizeClaim(player,
                            claim,
                    ...
                    if (!claim.isAdminClaim() && (player.hasPermission("griefprevention.deleteclaims") || player.hasPermission(
            ...
            else if (materialInHand != plugin.getPluginConfig().getClaimConfiguration().getToolsConfiguration().getModificationTool() || hand != EquipmentSlot.HAND)
                return;
            ...
            if (playerData.claimResizing != null && playerData.claimResizing.inDataStore) {
                ...
                newy2 = clickedBlock.getY() - plugin.getPluginConfig().getClaimConfiguration().getCreationConfiguration().extendIntoGroundDistance;
            ...
            if (claim != null) {
                ...
                Supplier<String> noEditReason = claim.checkPermission(player,
                        ClaimPermission.Edit,
                        event,
                        () -> plugin.dataStore.getMessage(Messages.CreateClaimFailOverlapOtherPlayer,
                                claim.getOwnerName()
                        )
                );
                if (noEditReason == null) {
                    ...
                    //if he didn't click on a corner and is in subdivision mode, he's creating a new subdivision
                        ...
                        //otherwise, he's trying to finish creating a subdivision by setting the other boundary corner
                            ...
                            CreateClaimResult result = dataStore.createClaim(player.getWorld(),
                                    playerData.lastShovelLocation.getBlockX(),
                                    clickedBlock.getX(),
                                    playerData.lastShovelLocation.getBlockY() - plugin.getPluginConfig().getClaimConfiguration().getCreationConfiguration().extendIntoGroundDistance,
                                    clickedBlock.getY() - plugin.getPluginConfig().getClaimConfiguration().getCreationConfiguration().extendIntoGroundDistance,
            ...
            if (lastShovelLocation == null) {
                ...
                if (plugin.getPluginConfig().getClaimConfiguration().getCreationConfiguration().maximumClaims > 0 && !player.hasPermission(
                        "griefprevention.overrideclaimcountlimit") && playerData.getClaims().size() >= plugin.getPluginConfig().getClaimConfiguration().getCreationConfiguration().maximumClaims) {
                ...
                BoundaryVisualization.visualizeArea(player,
            ...
            //otherwise, he's trying to finish creating a claim by setting the other boundary corner
                ...
                if (playerData.shovelMode != ShovelMode.Admin) {
                    if (newClaimWidth < plugin.getPluginConfig().getClaimConfiguration().getCreationConfiguration().minimumWidth || newClaimHeight < plugin.getPluginConfig().getClaimConfiguration().getCreationConfiguration().minimumWidth) {
                        ...
                        if (newClaimWidth != 1 && newClaimHeight != 1) {
                            GriefPrevention.sendMessage(player,
                                    ...
                                    String.valueOf(plugin.getPluginConfig().getClaimConfiguration().getCreationConfiguration().minimumWidth)
                            );
                        }
                        return;
                    }
                    ...
                    if (newArea < plugin.getPluginConfig().getClaimConfiguration().getCreationConfiguration().minimumArea) {
                        if (newArea != 1) {
                            GriefPrevention.sendMessage(player,
                                    TextMode.Err,
                                    Messages.ResizeClaimInsufficientArea,
                                    String.valueOf(plugin.getPluginConfig().getClaimConfiguration().getCreationConfiguration().minimumArea)
                            );
                        }
                ...
                CreateClaimResult result = dataStore.createClaim(player.getWorld(),
                        ...
                        clickedBlock.getX(),
                        lastShovelLocation.getBlockY() - plugin.getPluginConfig().getClaimConfiguration().getCreationConfiguration().extendIntoGroundDistance,
                        clickedBlock.getY() - plugin.getPluginConfig().getClaimConfiguration().getCreationConfiguration().extendIntoGroundDistance,
