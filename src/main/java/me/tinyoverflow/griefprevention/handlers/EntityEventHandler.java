...
public class EntityEventHandler implements Listener
{
    ...
    public void onEntityFormBlock(EntityBlockFormEvent event)
    {
        ...
        if (entity.getType() == EntityType.PLAYER)
        {
            ...
            String noBuildReason = plugin.allowBuild(player, event.getBlock().getLocation(), event.getNewState().getType());
    ...
    {
        if (!plugin.config_endermenMoveBlocks && event.getEntityType() == EntityType.ENDERMAN)
        {
            event.setCancelled(true);
        }
        else if (!plugin.config_silverfishBreakBlocks && event.getEntityType() == EntityType.SILVERFISH)
        {
            event.setCancelled(true);
        }
        else if (!plugin.config_rabbitsEatCrops && event.getEntityType() == EntityType.RABBIT)
        {
            event.setCancelled(true);
        }
        else if (!plugin.config_claims_ravagersBreakBlocks && event.getEntityType() == EntityType.RAVAGER)
        {
            event.setCancelled(true);
        }
        // All other handling depends on claims being enabled.
        else if (plugin.config_claims_worldModes.get(event.getBlock().getWorld()) == ClaimsMode.Disabled)
        {
            return;
        }
    ...
    private void handleProjectileChangeBlock(EntityChangeBlockEvent event, Projectile projectile)
    {
        ...
        if (claim == null)
        {
            ...
            if (plugin.creativeRulesApply(block.getLocation()) || plugin.config_claims_worldModes.get(block.getWorld()) == ClaimsMode.SurvivalRequiringClaims)
            {
                event.setCancelled(true);
                return;
            }
    ...
    static boolean isBlockSourceInClaim(@Nullable ProjectileSource projectileSource, @Nullable Claim claim)
    {
        return projectileSource instanceof BlockProjectileSource &&
                GriefPrevention.instance.getDataStore().getClaimAt(((BlockProjectileSource) projectileSource).getBlock().getLocation(), false, claim) == claim;
    }
    ...
    {
        if (!plugin.config_zombiesBreakDoors) event.setCancelled(true);
    }
    ...
    public void onEntityInteract(EntityInteractEvent event)
    {
        ...
        {
            if (!plugin.config_creaturesTrampleCrops)
            {
                event.setCancelled(true);
            }
            else
    ...
    void handleExplosion(Location location, Entity entity, List<Block> blocks)
    {
        ...
        boolean applySurfaceRules = world.getEnvironment() == Environment.NORMAL && ((isCreeper && plugin.config_blockSurfaceCreeperExplosions) || (!isCreeper && plugin.config_blockSurfaceOtherExplosions));
    ...
    private void preventPvpSpawnCamp(@NotNull EntityPickupItemEvent event, @Nullable Player player)
    {
        ...
        if (plugin.config_pvp_protectFreshSpawns && (plugin.getItemInHand(player, EquipmentSlot.HAND).getType() == Material.AIR))
        {
