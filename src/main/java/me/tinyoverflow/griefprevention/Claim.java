...
import me.tinyoverflow.griefprevention.listeners.block.BlockPlaceListener;
...
public class Claim {
    ...
    public Claim(Location lesserBoundaryCorner, Location greaterBoundaryCorner, UUID ownerID, List<String> builderIDs, List<String> containerIDs, List<String> accessorIDs, List<String> managerIDs, boolean inheritNothing, Long id) {
        ...
        for (String builderID : builderIDs) {
            setPermission(builderID, ClaimPermission.Build);
        }

        for (String containerID : containerIDs) {
            setPermission(containerID, ClaimPermission.Inventory);
        }

        for (String accessorID : accessorIDs) {
            setPermission(accessorID, ClaimPermission.Access);
        }
    ...
    public Claim(Claim claim) {
        ...
        lesserBoundaryCorner = claim.greaterBoundaryCorner.clone();
        greaterBoundaryCorner = claim.greaterBoundaryCorner.clone();
        id = claim.id;
        ownerID = claim.ownerID;
        managers = new ArrayList<>(claim.managers);
        playerIDToClaimPermissionMap = new HashMap<>(claim.playerIDToClaimPermissionMap);
        inDataStore = false; //since it's a copy of a claim, not in datastore!
        areExplosivesAllowed = claim.areExplosivesAllowed;
        parent = claim.parent;
        inheritNothing = claim.inheritNothing;
        children = new ArrayList<>(claim.children);
        siegeData = claim.siegeData;
        doorsOpen = claim.doorsOpen;
    }
    ...
    public boolean canSiege(Player defender) {
        ...
        return checkPermission(defender, ClaimPermission.Access, null) == null;
    }
    ...
    public void removeSurfaceFluids(Claim exclusionClaim) {
        ...
        if (getArea() > 10000) return;

        //only in creative mode worlds
        if (!GriefPrevention.instance.creativeRulesApply(lesserBoundaryCorner)) return;

        Location lesser = getLesserBoundaryCorner();
        Location greater = getGreaterBoundaryCorner();
    ...
    public int getArea() {
        int claimWidth = greaterBoundaryCorner.getBlockX() - lesserBoundaryCorner.getBlockX() + 1;
        int claimHeight = greaterBoundaryCorner.getBlockZ() - lesserBoundaryCorner.getBlockZ() + 1;

        return claimWidth * claimHeight;
    }

    public int getWidth() {
        return greaterBoundaryCorner.getBlockX() - lesserBoundaryCorner.getBlockX() + 1;
    }

    public int getHeight() {
        return greaterBoundaryCorner.getBlockZ() - lesserBoundaryCorner.getBlockZ() + 1;
    }
    ...
    public boolean isNear(Location location, int howNear) {
        Claim claim = new Claim
                (new Location(
                        lesserBoundaryCorner.getWorld(),
                        lesserBoundaryCorner.getBlockX() - howNear,
                        lesserBoundaryCorner.getBlockY(),
                        lesserBoundaryCorner.getBlockZ() - howNear
                ),
                        new Location(
                                greaterBoundaryCorner.getWorld(),
                                greaterBoundaryCorner.getBlockX() + howNear,
                                greaterBoundaryCorner.getBlockY(),
                                greaterBoundaryCorner.getBlockZ() + howNear
                        ),
    ...
    public boolean hasExplicitPermission(@NotNull UUID uuid, @NotNull ClaimPermission level) {
        if (uuid.equals(getOwnerID())) return true;

        if (level == ClaimPermission.Manage) return managers.contains(uuid.toString());

        return level.isGrantedBy(playerIDToClaimPermissionMap.get(uuid.toString()));
    }
    ...
    public boolean hasExplicitPermission(@NotNull Player player, @NotNull ClaimPermission level) {
        ...
        if (hasExplicitPermission(player.getUniqueId(), level)) return true;

        // Special case managers - a separate list is used.
        if (level == ClaimPermission.Manage) {
            for (String node : managers) {
        ...
        for (Map.Entry<String, ClaimPermission> stringToPermission : playerIDToClaimPermissionMap.entrySet()) {
    ...
    private @Nullable Supplier<String> getDefaultDenial(
        ...
        if (player != null) {
            if (hasExplicitPermission(player, permission)) return null;
        }
        else {
            if (hasExplicitPermission(uuid, permission)) return null;
        }

        // Check for public permission.
        if (permission.isGrantedBy(playerIDToClaimPermissionMap.get("public"))) return null;
        ...
        if (permission == ClaimPermission.Build) {
            ...
            if (material != null && placeableForFarming(material)
                    && getDefaultDenial(player, uuid, ClaimPermission.Inventory, event) == null)
                return null;
        }
        ...
        if (parent != null) {
            if (!inheritNothing)
                return parent.getDefaultDenial(player, uuid, permission, event);
        }

        // Catch-all error message for all other cases.
        return () ->
        {
            String reason = GriefPrevention.instance.dataStore.getMessage(
                    permission.getDenialMessage(),
                    getOwnerName()
            );
    ...
    public @Nullable ClaimPermission getPermission(@Nullable String playerID) {
        ...
        return playerIDToClaimPermissionMap.get(playerID.toLowerCase());
    }
    ...
    public void setPermission(@Nullable String playerID, @Nullable ClaimPermission permissionLevel) {
        ...
        if (permissionLevel == null) {
            ...
            managers.add(playerID.toLowerCase());
        }
        else {
            playerIDToClaimPermissionMap.put(playerID.toLowerCase(), permissionLevel);
        }
    }
    ...
    public void dropPermission(@NotNull String playerID) {
        ...
        playerIDToClaimPermissionMap.remove(playerID);
    ...
    public void clearPermissions() {
        playerIDToClaimPermissionMap.clear();
        managers.clear();
    ...
    public Location getLesserBoundaryCorner() {
        return lesserBoundaryCorner.clone();
    }
    ...
    public Location getGreaterBoundaryCorner() {
        return greaterBoundaryCorner.clone();
    }
    ...
    public String getOwnerName() {
        ...
        return GriefPrevention.lookupPlayerName(ownerID);
    }
    ...
    public boolean contains(Location location, boolean ignoreHeight, boolean excludeSubdivisions) {
        ...
        if (!Objects.equals(location.getWorld(), lesserBoundaryCorner.getWorld())) return false;
        ...
        if (parent != null) {
            return parent.contains(location, ignoreHeight, false);
        }
    ...
    public boolean overlaps(Claim otherClaim) {
        if (!Objects.equals(lesserBoundaryCorner.getWorld(), otherClaim.getLesserBoundaryCorner().getWorld()))
            return false;
    ...
    public String allowMoreEntities(boolean remove) {
        if (parent != null) return parent.allowMoreEntities(remove);

        //this rule only applies to creative mode worlds
        if (!GriefPrevention.instance.creativeRulesApply(getLesserBoundaryCorner())) return null;

        //admin claims aren't restricted
        if (isAdminClaim()) return null;

        //don't apply this rule to very large claims
        if (getArea() > 10000) return null;

        //determine maximum allowable entity count, based on claim size
        int maxEntities = getArea() / 50;
    ...
    public String allowMoreActiveBlocks() {
        if (parent != null) return parent.allowMoreActiveBlocks();

        //determine maximum allowable entity count, based on claim size
        int maxActives = getArea() / 100;
        if (maxActives == 0)
        ...
        for (Chunk chunk : chunks) {
            ...
            for (BlockState active : actives) {
                if (BlockPlaceListener.isActiveBlock(active)) {
                    if (contains(active.getLocation(), false, false)) {
                        totalActives++;
                    }
                }
            }
    ...
    boolean greaterThan(Claim otherClaim) {
        Location thisCorner = getLesserBoundaryCorner();
    ...
    public long getPlayerInvestmentScore() {
        ...
        Set<Material> playerBlocks = RestoreNatureProcessingTask.getPlayerBlocks(
                lesserBoundaryCorner.getWorld().getEnvironment(),
        ...
        for (int x = this.lesserBoundaryCorner.getBlockX(); x <= greaterBoundaryCorner.getBlockX(); x++) {
            for (int z = this.lesserBoundaryCorner.getBlockZ(); z <= greaterBoundaryCorner.getBlockZ(); z++) {
    ...
    public ArrayList<Chunk> getChunks() {
        ...
        Chunk lesserChunk = getLesserBoundaryCorner().getChunk();
        Chunk greaterChunk = getGreaterBoundaryCorner().getChunk();
