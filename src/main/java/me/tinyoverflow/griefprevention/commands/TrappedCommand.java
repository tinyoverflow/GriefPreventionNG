...
public class TrappedCommand extends BaseCommand implements PlayerCommandExecutor
{
    ...
    public void run(Player player, CommandArguments commandArguments) throws WrapperCommandSyntaxException
    {
        ...
        Claim claim = getPlugin().getDataStore().getClaimAt(player.getLocation(), false, playerData.lastClaim);
        ...
        if (!GriefPrevention.instance.getPluginConfig().getClaimConfiguration().isAllowTrappedInAdminClaims() &&
            claim.isAdminClaim() && event.getDestination() == null)
        {
            GriefPrevention.sendMessage(player, TextMode.ERROR, Messages.TrappedWontWorkHere);
            return;
        }
        //send instructions
        GriefPrevention.sendMessage(player, TextMode.INSTRUCTION, Messages.RescuePending);
